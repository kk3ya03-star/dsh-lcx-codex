...
test('Chinese and English README files link to each other and keep the same safety boundary', () => {
  ...
  assert.match(englishReadme, /caller of Hosted or Alpha does not need to be GPT/iu)
  assert.match(englishReadme, /temporarily selects the plugin search provider/iu)
  assert.match(englishReadme, /selected GPT provider's `apiKeyEnv`/iu)
...
test('public wording identifies the compatibility boundary without official affiliation claims', () => {
  ...
  assert.match(readme, /Hosted\/Alpha 在当前主模型本身是兼容 GPT Responses 路由时复用该路由，否则使用插件设置中的 GPT Responses 路由/iu)
  assert.match(readme, /Native Compact 只复用当前 GPT 主模型的 provider、model、Responses 地址、凭据引用/iu)
  ...
  assert.match(readme, /dsh plugin --profile web add \.\\dsh-lcx-codex-0\.3\.4\.tgz/iu)
  ...
  assert.match(readme, /调用 Hosted\/Alpha 的主模型不限于 GPT/iu)
  assert.match(readme, /临时切换为插件搜索 provider/iu)
  assert.match(readme, /搜索网络目标优先由当前兼容的 DSH `openai-responses` provider 决定/iu)
  assert.match(readme, /不固定到 LCX 或其他域名/iu)
  assert.match(readme, /只在该地址下调用 `\/responses` 和 `\/alpha\/search`/iu)
  assert.match(readme, /凭据名称取自最终选中的 GPT provider 的 `apiKeyEnv`/iu)
