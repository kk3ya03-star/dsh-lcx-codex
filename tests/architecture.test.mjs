...
test('package targets DSH 0.1.1-rc.2 directly', async () => { const pkg = JSON.parse(await text('package.json')); assert.equal(pkg.version, '0.4.0'); assert.equal(pkg.dependencies['@earendil-works/pi-ai'], '0.82.1'); assert.equal(pkg.devDependencies['@deepseek-ai/dsh-llm'], '0.1.1-rc.2'); assert.equal(pkg.peerDependencies['@deepseek-ai/dsh-attachment'], '^0.1.1-rc.2'); assert.equal(pkg.peerDependencies['@deepseek-ai/dsh-compaction-basic'], undefined); assert.equal(pkg.peerDependencies['@deepseek-ai/dsh-web'], undefined) })
