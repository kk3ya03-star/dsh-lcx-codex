...
test('package targets DSH 0.1.1-rc.2 directly', async () => { const pkg = JSON.parse(await text('package.json')); assert.equal(pkg.version, '0.4.0-rc.13'); assert.equal(pkg.dependencies['@earendil-works/pi-ai'], '0.82.1'); assert.equal(pkg.devDependencies['@deepseek-ai/dsh-llm'], '0.1.1-rc.2'); assert.equal(pkg.peerDependencies['@deepseek-ai/dsh-attachment'], '^0.1.1-rc.2'); assert.equal(pkg.peerDependencies['@deepseek-ai/dsh-compaction-basic'], undefined); assert.equal(pkg.peerDependencies['@deepseek-ai/dsh-web'], undefined) })
test('Cordis service access is guarded by explicit runtime inject and default-export metadata', async () => { const source = await text('lib/index.js'); assert.match(source, /export const inject = \['llm', 'web', 'sessions'\]/u); assert.match(source, /ctx\.inject\(\['llm', 'web', 'sessions'\], injectedCtx => installInjected\(injectedCtx, configInput\)\)/u); assert.match(source, /apply\.inject = inject/u); assert.match(source, /apply\.Config = Config/u) })
...
test('web_search deadline is extended without changing its model schema', async () => { const source = await text('lib/index.js'); assert.match(source, /definition\.timeoutMs/u); assert.match(source, /webSearchTimeoutSeconds/u); const client = await text('lib/client.js'); assert.match(client, /240/u) })
...
test('Native-first pressure patch deduplicates on the concrete compaction service identity', async () => {
  ...
  assert.match(source, /const compaction = concreteService\(compactionValue\)/u)
...
test('isolated pressure coordination uses the official AgentPresets service resolver', async () => {
  const source = await text('lib/index.js')
  assert.match(source, /agentPresets/u)
  assert.match(source, /serviceFor\?\.\(agent, name\)/u)
...
test('request-header cache seeds resumed sessions and releases disposed sessions', async () => {
  ...
  assert.match(source, /ctx\.on\('session\/disposed', \(session\) => \{\s*requestHeaders\.delete\(String\(session\?\.id \?\? ''\)\)\s*\}, \{ global: true \}\)/u)
})
