...
const DSH_PROJECTION_UNAVAILABLE_SUFFIX = '" was removed or replaced; restore the session with its owning plugin';
/**
 ...
 * @param {unknown} cause
 */
function isMissingOrReplacedProjectionError(cause) {
    if (!(cause instanceof Error))
        return false;
    const { message } = cause;
    if (!message.startsWith(DSH_PROJECTION_UNAVAILABLE_PREFIX) ||
        !message.endsWith(DSH_PROJECTION_UNAVAILABLE_SUFFIX))
        return false;
    const projectionType = message.slice(DSH_PROJECTION_UNAVAILABLE_PREFIX.length, message.length - DSH_PROJECTION_UNAVAILABLE_SUFFIX.length);
    return projectionType.length > 0;
}
function checkpointHistoryReadError(cause) {
    return Object.assign(new Error("Unsupported or invalid LCX checkpoint history; start a new session", {
        cause,
    }), { code: "LCX_CHECKPOINT_UNSUPPORTED" });
}
/**
...
function eventMessage(session, seq) {
    if (!Number.isSafeInteger(seq) || seq < 0)
        throw checkpointHistoryReadError(new Error("Portable checkpoint history contains an invalid event sequence"));
    try {
        ...
        if (isMissingOrReplacedProjectionError(cause))
            throw Object.assign(new Error("Session history requires a message projection this runtime does not provide; restore the owning plugin to reuse this checkpoint", { cause }), { code: "LCX_CHECKPOINT_PROJECTION_UNAVAILABLE" });
