...
function requestImageOptions(routeConfig, imageSupport, signal, imageMap) {
  return {
...
async function serializeNativeAware(messages, route, routeConfig, ctx, options = {}) {
  ...
  const input = []; const imageMap = new Map(); let normal = []
  const flush = async () => {
    if (!normal.length) return
    const serialized = await serializeDshMessages(normal, ctx, requestImageOptions(routeConfig, imageSupport, options.signal))
      ...
      if (stateRouteCompatible(state, route, ctx)) {
        ...
        if (state.version === 4) {
          ...
          if (shadowed.length > 0) {
            const serializedShadowed = await serializeDshMessages(shadowed, ctx, requestImageOptions(routeConfig, imageSupport, options.signal))
            ...
            if (repairedRetained.length > 0 && opaque) {
              ...
              const serializedPortable = await serializeDshMessages(portable, ctx, requestImageOptions(routeConfig, imageSupport, options.signal))
        ...
        const serialized = await serializeDshMessages(portable, ctx, requestImageOptions(routeConfig, imageSupport, options.signal))
  ...
  await flush()
  return { input, imageMap, imageSupport }
}
...
async function* remoteCompactionStream(options, routeConfig, state, ctx, next) {
  const history = stripCompactionDirective(options.messages)
  const route = currentRoute(options, routeConfig)
  try {
    const prepared = await serializeNativeAware(history, route, routeConfig, ctx, { signal: options.signal })
    const headers = await authenticatedHeaders(ctx, routeConfig, route.sessionId)
    ...
    ctx.logger?.warn?.(`[lcx-codex] native V2 compaction failed: ${error?.code ?? error?.name ?? 'ERROR'} ${error?.message ?? String(error)}`)
    if (!state.fallbackToBasicCompaction || !fallbackEligible(error, options.signal)) throw error
    ctx.logger?.info?.('[lcx-codex] falling back to DSH basic compaction after native failure')
...
function resolveScopedService(agent, name) {
  try { return agent?.ctx?.get?.(name) ?? agent?.ctx?.[name] } catch { return undefined }
}

function patchCompactionPressureForAgent(agent, state, getConfig, ctx, records) {
  const compaction = resolveScopedService(agent, 'compaction')
  if (!compaction || typeof compaction.compactIfNeeded !== 'function' || records.has(compaction)) return
  const original = compaction.compactIfNeeded
  const record = { compaction, original, wrapper: undefined }
  const wrapper = async function(agentArg, trigger, signal) {
    if (trigger !== 'pressure' || !state.enabled || !state.remoteCompaction || !state.autoCompaction) return original.call(this, agentArg, trigger, signal)
    const config = getConfig()
    const target = routedTargetForAgent(agentArg, config)
    if (!target || !resolveResponsesRouteConfig(ctx, target, config)) return original.call(this, agentArg, trigger, signal)
    const tokenMeter = resolveScopedService(agentArg, 'tokenMeter') ?? this?.ctx?.get?.('tokenMeter')
    const llm = resolveScopedService(agentArg, 'llm') ?? ctx?.get?.('llm') ?? ctx?.llm
    if (!tokenMeter?.measure || !llm?.resolveModelInfo) return original.call(this, agentArg, trigger, signal)
    let contextWindow
    try { contextWindow = (await llm.resolveModelInfo(target.provider, target.model, signal))?.context?.contextWindow } catch { return original.call(this, agentArg, trigger, signal) }
    if (!Number.isFinite(contextWindow) || contextWindow <= 0) return original.call(this, agentArg, trigger, signal)
    ...
    const pruner = resolveScopedService(agentArg, 'toolResultPruner') ?? this?.ctx?.get?.('toolResultPruner')
    const originalPrune = pruner?.pruneSession
    const originalConfig = this?.config
    const nativeFirst = ratioPercent < emergency
    if (nativeFirst && pruner && typeof originalPrune === 'function') pruner.pruneSession = () => ({ pruned: [], charsRemoved: 0 })
    if (originalConfig && this && Object.prototype.hasOwnProperty.call(this, 'config')) { try { this.config = adjustedCompactionConfig(originalConfig, target, auto / 100) } catch {} }
    ...
    try { return await original.call(this, agentArg, trigger, signal) }
    finally {
      if (originalConfig && this && this.config !== originalConfig) { try { this.config = originalConfig } catch {} }
      if (nativeFirst && pruner && typeof originalPrune === 'function' && pruner.pruneSession !== originalPrune) { try { pruner.pruneSession = originalPrune } catch {} }
    }
  }
  record.wrapper = wrapper
  try { compaction.compactIfNeeded = wrapper } catch { return }
  records.set(compaction, record)
}

function restoreCompactionPressure(records) {
  for (const record of records.values()) if (record.compaction?.compactIfNeeded === record.wrapper) { try { record.compaction.compactIfNeeded = record.original } catch {} }
  records.clear()
}
...
async function* replayCheckpointStream(options, routeConfig, ctx) {
  ...
  const prepared = await serializeNativeAware(options.messages, route, routeConfig, ctx, { signal: options.signal })
  const headers = await authenticatedHeaders(ctx, routeConfig, route.sessionId)
  yield* requestNativeReplay({ baseURL: routeConfig.baseURL, model: route.model, input: prepared.input, system: options.system, tools: options.tools, promptCacheKey: promptCacheKey(route), headers, signal: options.signal, timeoutMs: routeConfig.timeoutMs, maxAttempts: routeConfig.maxAttempts, maxResponseBytes: routeConfig.maxResponseBytes })
}
...
function installInjected(ctx, configInput = {}) {
  ...
  ctx.on('agent/status', ({ agent, status }) => {
    ...
    patchCompactionPressureForAgent(agent, state, () => runtimeConfig, ctx, compactionPatchRecords)
  ...
  ctx.on('llm/stream', (options, next) => {
    ...
    if (options.purpose === 'compaction') {
      ...
      return remoteCompactionStream(options, routeConfig, state, ctx, next)
    }
  ...
  ctx.effect?.(() => () => {
    ...
    restoreCompactionPressure(compactionPatchRecords)
...
export function apply(ctx, configInput = {}) {
  return ctx.inject(['llm', 'web'], injectedCtx => installInjected(injectedCtx, configInput))
}
