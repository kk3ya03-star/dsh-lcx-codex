...
function installInjected(ctx, configInput = {}) {
    ...
    installSettingsCompat(ctx, SETTINGS_NS, SettingsSchema, settingsEntry, {
        setSource(current) {
            source = current;
        },
        onChange() {
     ...
     */
    const instrumentAgent = (agent, edge) => {
        try {
            ...
            ctx.logger?.warn?.(`[lcx-codex] agent instrumentation skipped on ${edge}: ${String(error)}`);
        }
    };
    /** DSH 0.1.6 lifecycle source, recorded for diagnostics only. */
    const startSource = (value) => typeof value === "string" && /^[a-z]+$/u.test(value) ? value : "unknown";
    ctx.on("agent/created", (payload) => {
        ...
        instrumentAgent(agent, `agent/created(${startSource(payload?.source)})`);
    }, { global: true });
