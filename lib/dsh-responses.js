import { IMAGE_OFFLOAD_REQUIRED_CODE, LlmError, offloadedImageText, projectOffloadedImages, requiredImageOffload, requestImageHandleText, resolveImageAttachmentAccess, } from "@deepseek-ai/dsh-llm";
import { requestImageDimensions } from "@deepseek-ai/dsh-attachment";
import { convertResponsesMessages, convertResponsesTools, createGrammarToolInputProperties, getBuiltinModels, normalizeContext, } from "./pi-responses-runtime.js";
...
function attachmentResolver(ctx, options, requestImages) {
    ...
    return async (block, signal) => {
        ...
        const request = requestImages === undefined
            ? await attachments.store.readImageRequest(block.attachment, { ...requestImageDimensions(block.attachment.width, block.attachment.height, options.requestImagePixelBudget), maxBytes: options.requestImageMaxBytes }, signal)
...
async function imagePart(block, ctx, options, imageMap, requestImages) {
...
async function piImageParts(block, ctx, options, imageMap, requestImages) {
    if (options.imageSupport === "unsupported")
...
async function piToolContent(blocks, ctx, options, imageMap, requestImages) {
    const content = [];
    for (const block of blocks) {
        if (block.type === "text")
            content.push({ type: "text", text: block.text });
        else if (block.type === "image")
            content.push(...(await piImageParts(block, ctx, options, imageMap, requestImages)));
        else if (block.type === "reasoning")
            continue;
        else if (block.type === "tool-result")
            content.push(...(await piToolContent(block.content, ctx, options, imageMap, requestImages)));
        else
            throw unsupportedContent(block.type);
    }
...
function collectRequestImageRefs(blocks, refs) {
    for (const block of blocks) {
        if (block.type === "image") {
...
async function prepareRequestImageVersions(messages, ctx, options) {
    const attachments = attachmentServices(ctx?.attachments);
    if (attachments === undefined)
        return new Map();
    const refs = new Map();
    for (const message of messages)
        collectRequestImageRefs(message.content, refs);
    const versions = await Promise.all([...refs.values()].map(async (ref) => {
        ...
        return [String(ref.attachmentId), await attachments.store.readImageRequest(ref, target, options.signal)];
    }));
...
async function dshToPiMessages(messages, ctx, options, imageMap, requestImages) {
    const result = [];
...
export async function serializeDshMessages(messages, ctx, options = {}) {
    ...
    const offloadImages = attachments === undefined || normalized.imageSupport === "unsupported"
        ? 0
        : requiredImageOffload(fileProjected, { representation: "base64", maxBytes: normalized.maxRequestImageBytes }, (block) => {
    ...
    if (offloadImages > 0) {
        throw new LlmError(`LCX request images exceed the configured base64 bound; ${offloadImages} more oldest occurrence(s) must be offloaded.`, IMAGE_OFFLOAD_REQUIRED_CODE, { offloadImages });
    }
    const projected = projectOffloadedImages(fileProjected, (ref) => offloadedImageText(ref, attachments === undefined ? undefined : imageAccess(ctx, attachments, ref)));
    const model = resolvePiResponsesModel(normalized);
    const dshContext = { systemPrompt: normalized.systemPrompt, messages: await dshToPiMessages(projected, ctx, normalized, imageMap, requestImages), tools: [...normalized.tools] };
    ...
    for (const tool of dshContext.tools)
        if (tool.name)
