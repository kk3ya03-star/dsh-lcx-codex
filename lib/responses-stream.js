 ...
 */
function imageOffloadFailure(error) {
    if (!isObject(error))
        return undefined;
    const source = isObject(error.failure) ? error.failure : error;
    if (source.code !== IMAGE_OFFLOAD_REQUIRED_CODE)
        return undefined;
    const offloadImages = source.offloadImages;
    if (!Number.isSafeInteger(offloadImages) || offloadImages <= 0)
        return undefined;
    const message = typeof source.message === "string" && source.message.length > 0
        ? source.message
        : "Request images exceed the route budget";
    return { message, code: IMAGE_OFFLOAD_REQUIRED_CODE, offloadImages: offloadImages };
}
        ...
        try {
            while (true) {
                ...
                if (watchdog.signal.aborted)
                    throw abortError(watchdog.signal);
                if (result.done) {
                    exhausted = true;
                    watchdog.disarm();
                    break;
