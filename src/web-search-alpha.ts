...
type AlphaResponse = { output: string; results?: unknown[]; id?: unknown };
...
function failure(message: string | undefined, code = "WEB_INVALID_REQUEST"): Error & { code: string } {
  return Object.assign(new Error(message), { code });
}
function isObject(v: unknown): v is RecordValue {
...
function integer(value: unknown, field: string, min = 0, max = 10000): number {
  if (
    typeof value !== "number" ||
...
export function normalizeAlphaSearchArgs(args: unknown): NormalizedAlphaArgs {
  ...
  if (args.responseLength !== undefined) {
    if (
      typeof args.responseLength !== "string" ||
      !(RESPONSE_LENGTHS as readonly string[]).includes(args.responseLength)
    )
  ...
  switch (args.action) {
    ...
    case "click":
      ...
      if (isAlphaHttpUrl(result.refId))
        throw failure(
          "websearch_alpha.click requires an opaque stored ref",
          "LCX_ALPHA_REF_REQUIRED",
        );
    ...
    case "finance":
      ...
      if (
        typeof args.assetType !== "string" ||
        !(ASSET_TYPES as readonly string[]).includes(args.assetType)
      )
...
export function alphaActionCommand(args: NormalizedAlphaArgs): Record<string, unknown> {
  let value;
  switch (args.action) {
    case "search_query":
    case "image_query":
...
export function buildAlphaSearchBody(
  ...
  if (typeof sessionId !== "string" || !sessionId)
    throw failure(
      "websearch_alpha requires a DSH session",
...
function ipv4Octets(hostname: string) {
  const octets: number[] = hostname.split(".").map((part) => Number(part));
...
export function isPublicAlphaTarget(url: URL) {
...
export function isAlphaHttpUrl(value: unknown): value is string {
...
export function isAlphaContinuationUrl(value: unknown): value is string {
...
export function alphaRefRequiresStore(action: string, refId: unknown) {
  return (
    action === "click" ||
...
function safeResult(value: unknown, seen = new Set<object>()): unknown {
...
export function parseAlphaSearchResponse(
  ...
  if (
    ...
    throw failure(
      "LCX Alpha Web Search returned an invalid response",
  ...
  if (
    ...
    throw failure(
      "LCX Alpha Web Search could not execute the requested action",
  ...
  for (const source of outputBlocks.flatMap((b) =>
    ...
    if (!a.sources.some((x) => x.url === source.url)) a.sources.push(source);
  return {
    mode: "alpha",
    ...
    warnings:
      options.capability === "command-capable"
        ? [
...
function probeFailureState(error: unknown): "unsupported" | "unknown" {
  ...
  if (
    [404, 405].includes(detail.status as number) ||
    ["LCX_ALPHA_ACTION_UNAVAILABLE", "LCX_ALPHA_ACTION_FAILED"].includes(String(detail.code ?? "")) ||
    /channel does not support|unsupported action|not implemented/iu.test(
...
function alphaProbeStructuredResult(value: unknown, seen = new Set<object>()): boolean {
  ...
  if (
    ["line", "line_number", "page", "page_number", "pageno"].some((field) =>
      Number.isSafeInteger(value[field]),
    )
  )
    return true;
  return Object.values(value).some((item) =>
    alphaProbeStructuredResult(item, seen),
  );
}
function alphaProbeStructuredBlock(block: unknown): boolean {
...
const ALPHA_PROBE_SEMANTIC_FAILURE =
...
function alphaProbeResultHasEvidence(action: string, value: unknown): boolean {
  ...
  const refs =
    ...
    data.refs.some(
      (ref: unknown) => typeof ref === "string" && ref && !isAlphaHttpUrl(ref),
    );
  const links =
    Array.isArray(data.links) &&
    data.links.some((link) => isObject(link) && Number.isSafeInteger(link.id));
  const pdfRefs =
    Array.isArray(data.pdfRefs) &&
    data.pdfRefs.some(
      (ref: unknown) => typeof ref === "string" && ref && !isAlphaHttpUrl(ref),
    );
  ...
  const structuredBlocks =
    Array.isArray(data.outputBlocks) &&
  ...
  return Boolean(
    refs || links || pdfRefs || structuredResults || structuredBlocks,
  );
}
export async function probeAlphaCapabilities({
  ...
  const actions = Object.fromEntries(
    ALPHA_ACTIONS.map((action) => [action, "unknown"]),
  );
  const result = {
  ...
  for (const [action, args] of Object.entries(actionProbes)) {
    if (
      !["image_query", "finance", "weather", "sports", "time"].includes(action)
    )
      continue;
    try {
