...
import {
  buildCompactionResponsesBody,
  responsesGenerationEnvelope as standardGenerationEnvelope,
...
type FunctionCallItem = { type: "function_call"; call_id: string };
type FunctionCallOutputItem = { type: "function_call_output"; call_id: string };
type ValidatedOutputItem =
  | CompactionItem
  | FunctionCallItem
...
type SseParseOptions = { signal?: AbortSignal; maxResponseBytes?: number };
...
function fail(
  ...
  const e: LcxError = new Error(message);
  e.code = code;
  return e;
}
/** @param {unknown} value */
function safeMachineField(value: unknown): string | undefined {
 ...
 * @returns {HeaderMap}
 */
export function mergeFeatureHeader(headers: HeaderMap = {}): HeaderMap {
 ...
 */
export function buildNativeCompactionBody({
  model,
  modelDescriptor,
  input,
  tools,
 ...
 */
function usageFrom(raw: unknown): CanonicalUsage | undefined {
  if (!isObject(raw)) return undefined;
  const inputTotal = Number(raw.input_tokens ?? 0);
  const output = Number(raw.output_tokens ?? 0);
  const detailsValue =
    raw.input_tokens_details ?? raw.input_token_details ?? raw.prompt_tokens_details;
...
function validateOutput(output: unknown): {
  ...
  const compactions = output.filter(
    (item) => isObject(item) && item.type === "compaction",
  );
  if (compactions.length !== 1)
    throw fail(
      `native compaction must return exactly one compaction item, got ${compactions.length}`,
    );
  const compact = compactions[0]!;
  if (
    typeof compact.encrypted_content !== "string" ||
  ...
  for (const item of output) {
    ...
    if (item.type === "function_call") {
      if (
        ...
        throw fail(
          "native compaction output contains an invalid function_call",
        );
      ...
      if (
        ...
        throw fail(
          "native compaction output contains an invalid function_call_output",
        );
  ...
  for (const id of results)
    if (!calls.has(id))
      throw fail(
        `native compaction output has orphan function_call_output ${id}`,
      );
  return {
    output: structuredClone(output) as ValidatedOutputItem[],
    compaction: structuredClone(compact) as CompactionItem,
  };
}
...
export async function parseNativeCompactionSse(
    ...
    (event: unknown) => {
      ...
      if (
        ...
        const diagnostics = [
          ...
          .join(" ");
        const e = fail(
          `native compaction ended with ${event.type}${diagnostics ? ` ${diagnostics}` : ""}`,
          "LCX_COMPACT_UPSTREAM_ERROR",
        );
        if (
          Number.isInteger(
            upstream.status ?? (isObject(event.error) ? event.error.status : undefined),
          )
        )
          e.status = Number(
            upstream.status ?? (isObject(event.error) ? event.error.status : undefined),
  ...
  if (!isObject(terminal))
    throw fail(
      "native compaction stream ended without response.completed",
      "LCX_COMPACT_INCOMPLETE_SSE",
    );
  if (terminal.status !== "completed")
    throw fail(
      "native compaction response.completed did not carry completed status",
      "LCX_COMPACT_INCOMPLETE_SSE",
    );
...
export async function requestNativeCompaction({
  ...
  maxAttempts,
  maxResponseBytes,
}: NativeCompactionRequestOptions): Promise<NativeCompactionResult> {
  ...
  return fetchSseWithRetry<NativeCompactionResult>(
    ...
    timeoutMs,
    {
      ...
      consume: (
        response: Response,
        consumeOptions: { requestSignal?: AbortSignal },
      ) =>
        parseNativeCompactionSse(response, {
